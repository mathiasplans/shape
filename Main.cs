...
public class MainClass {
    ...
    public static void DrawSymbols(Graphics gr, List<IShape> shapes, Font font, Brush brush, float offx, float offy) {
        foreach (IShape shape in shapes) {
            ...
            gr.DrawString(shape.ToString(), font, brush, new PointF(center.x - offx, center.y - offy));
        }
    }

    public static void DrawGraph(Graphics gr, List<IShape> shapes, float factor) {
        Font symbolfont = new Font("Times New Roman", 10f * factor);
        ...
        foreach (ShapeGraph.Node node in sg) {
            ...
            // Draw the point
            gr.FillEllipse(brush2, new Rectangle((int) (nodeCenter.x - 8 * factor), (int) (nodeCenter.y - 8 * factor), (int) (17 * factor), (int) (17 * factor)));


            done.Add(node);
        }

        DrawSymbols(gr, shapes, symbolfont, brush, 3.5f * factor, 7f * factor);
    }
